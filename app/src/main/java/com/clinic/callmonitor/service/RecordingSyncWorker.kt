...
class RecordingSyncWorker(context: Context, params: WorkerParameters) : Worker(context, params) {
    ...
    companion object {
        ...
        /** تشغيل فوري بعد إغلاق مكالمة - بتأخير بسيط عشان سامسونج يخلص كتابة الملف */
    ...
    private fun uploadRecordingFile(match: DeviceRecordingFile, deviceUuid: String, employeeId: String) {
        val fileKey = "$KEY_RECORDING_UPLOADED_PREFIX${match.file.name}"
        if (CallMonitorApp.prefs.getBoolean(fileKey, false)) return

        try {
            val reqFile = match.file.asRequestBody("audio/mp4".toMediaTypeOrNull())
            ...
            val resp = ApiClient.service.uploadRecording(
                ...
                match.file.name.toRequestBody("text/plain".toMediaTypeOrNull()),
                filePart
            )
            if (resp.isSuccessful) {
