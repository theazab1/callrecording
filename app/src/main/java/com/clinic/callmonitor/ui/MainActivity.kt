...
class MainActivity : AppCompatActivity() {
    ...
    private fun uploadTestRecording(file: File, startTime: Long, button: Button, statusView: TextView) {
        ...
        if (token.isNullOrEmpty()) {
            statusView.text = "❌ التوكن غير محفوظ! يرجى حفظ التوكن أولاً ثم إعادة المحاولة."
            button.isEnabled = true
            return
        }
        ...
        CoroutineScope(Dispatchers.IO).launch {
            ...
            try {
                ...
                val resp = ApiClient.service.uploadRecording(
                    ...
                    file.name.toRequestBody("text/plain; charset=utf-8".toMediaTypeOrNull()),
                    filePart
                )
                success = resp.isSuccessful
                if (!success) errorMsg = "كود الخطأ: ${resp.code()} - ${resp.message()}"
            } catch (e: Exception) {
